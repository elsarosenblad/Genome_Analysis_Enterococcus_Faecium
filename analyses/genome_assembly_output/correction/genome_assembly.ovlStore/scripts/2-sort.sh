...
#  This script should be executed from correction/genome_assembly.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./genome_assembly.ovlStore.BUILDING/$jobname.info -a ! -e ./genome_assembly.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './genome_assembly.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./genome_assembly.ovlStore.BUILDING \
  -S ../genome_assembly.seqStore \
  -C  ./genome_assembly.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
